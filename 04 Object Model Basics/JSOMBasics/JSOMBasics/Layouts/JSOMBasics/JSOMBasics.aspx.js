...
function loadScript(script) {
    ...
    writeToLog(); //might give an error because not yet loaded
}
...
function exceptionHandling() {
    ...
    listCreationInformation.set_title(listName);
    listCreationInformation.set_templateType(template);
    listCreationInformation.set_description(desc);
    var list = cctx.get_web().get_lists().add(listCreationInformation);
    catchBlock.dispose();
...
function createWebsite() {
    ...
    webCreationInfo.set_title('JSOM Web Site');
    webCreationInfo.set_url('jsomweb');
...
function createContentType() {
    ...
    contentTypeCreationInformation.set_name('PartnerCompany');
    contentTypeCreationInformation.set_group('SmartPortal');
    contentTypeCreationInformation.set_description('Content Type for partner companies');
    contentTypeCreationInformation.set_parentContentType(parentCT);
    var ct = contentTypes.add(contentTypeCreationInformation);
    cctx.load(ct);
    cctx.executeQueryAsync(function () {
            ctID = ct.get_id();
    ...
    $.getScript(scriptbase + "sp.taxonomy.js",
        function () {
            ...
            cctx.executeQueryAsync(function () {
                ...
                if (tgTraining == null) {
                    tgTraining = termStore.createGroup(tgName, SP.Guid.newGuid().toString());
