(function () {
    ...
    eventsApp.config(["$routeProvider", "$httpProvider", "adalAuthenticationServiceProvider",
    ...
    eventsApp.controller('eventsController', function ($scope, $http, adalAuthenticationService) {

        if (adalAuthenticationService.userInfo.isAuthenticated) {
            $scope.message = "Welcome to Events.<br>You are logged in as " + adalAuthenticationService.userInfo.userName;
        }
        else {
            $location.path("/");
        }
